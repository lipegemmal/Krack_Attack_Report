...
O caso do KrackAttack que será abordado neste artigo diz respeito a um ato malicioso em que o dispositivo é induzido a mudança da chave de criptografia do cliente Wi-Fi em dispositivos Android e Linux  para zero (all zero key). Isso é feito manipulando-se o processo de \textit{4-way-handshake} entre um suplicante e um ponto de acesso, mais especificamente apos a terceira mensagem, onde é instalado uma chave, e caso uma resposta de confirmação não seja recebida, a terceira mensagem será reinviada e a chave reinstalada. Foi mostrado que um ataque pode forçar essas reinstalações da chave com uma coleta e repetição da terceira mensagem do \textit{4-way-handshake}. 
...
Note que tal ataque não pode resultar na obtenção da senha da rede utilizada, nem nas chaves de kriptografia usadas para o handshake. Também não é possível decriptar pacotes criptografados, e como a grande maioria da troca de pacotes em rede é criptografado, é difícil se encontrar numa situação onde esse ataque pode ser usado e obter algum valor significativoc como resultado
